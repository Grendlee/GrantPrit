...
void Hexify::generatePabloMethod() {
    ...
    // insertMask is the orig byte slot. shift it to find the slots after.
    PabloAST * oneByteAfterOriginal = pb.createAdvance(insertMask, 1);
    PabloAST * twoByteAfterOriginal = pb.createAdvance(insertMask, 2);

    PabloAST * insertedPosition = pb.createNot(insertMask);

    // first inserted slot, emit the low hex nibble
    PabloAST * lowNibblePosition = pb.createAnd(oneByteAfterOriginal, insertedPosition);

    // second inserted slot, emit the space character
    PabloAST * spaceCharPosition = pb.createAnd(twoByteAfterOriginal, pb.createAnd(insertedPosition, pb.createNot(oneByteAfterOriginal)));
    ...
    lo[3] = pb.createSel(insertMask, spreadBasis[7], pb.createSel(lowNibblePosition, pb.createAdvance(spreadBasis[3], 1), pb.createZeroes()), "lo[3]");
    
    lo[2] = pb.createSel(insertMask, spreadBasis[6], pb.createSel(lowNibblePosition, pb.createAdvance(spreadBasis[2], 1), pb.createZeroes()), "lo[2]");
    
    lo[1] = pb.createSel(insertMask, spreadBasis[5], pb.createSel(lowNibblePosition, pb.createAdvance(spreadBasis[1], 1), pb.createZeroes()), "lo[1]");
    
    lo[0] = pb.createSel(insertMask, spreadBasis[4], pb.createSel(lowNibblePosition, pb.createAdvance(spreadBasis[0], 1), pb.createZeroes()), "lo[0]");
    ...
    hexBasis[4] = pb.createSel(spaceCharPosition, pb.createZeroes(), pb.createNot(hexA_F));
    //
...
class HexSlotCount : public PabloKernel {

public:

    HexSlotCount(LLVMTypeSystemInterface & ts, StreamSet * insertCount, StreamSet * nonLF)
        : PabloKernel(ts, "HexSlotCount", {Binding{"nonLF", nonLF, FixedRate(), LookAhead(1)}}, {Binding{"insertCount", insertCount}}) {}
protected:

    void generatePabloMethod() override;
};

void HexSlotCount::generatePabloMethod() {
    pablo::PabloBuilder pb(getEntryScope());
    //

    PabloAST * currentIsNonLF = getInputStreamSet("nonLF")[0];
    PabloAST * currentIsLF = pb.createNot(currentIsNonLF);  

    PabloAST * nextsNonLF = pb.createLookahead(currentIsNonLF, 1);


    PabloAST * lastByteOfLine = pb.createAnd(currentIsNonLF, pb.createNot(nextsNonLF));
    PabloAST * middleByteOfLine = pb.createAnd(pb.createNot(currentIsLF), nextsNonLF);

    
    Var * insertCount = getOutputStreamVar("insertCount");

    pb.createAssign(pb.createExtract(insertCount, pb.getInteger(0)), lastByteOfLine);
    
    pb.createAssign(pb.createExtract(insertCount, pb.getInteger(1)), middleByteOfLine);
}
...
HexLinesFunctionType generatePipeline(CPUDriver & driver) {
    ...
    StreamSet * hexInsertCount = P.CreateStreamSet(2);
    P.CreateKernelCall<HexSlotCount>(hexInsertCount, nonLF);

    StreamSet * hexInsertMask = P.CreateStreamSet(1);
    InsertionSpreadMask(P, hexInsertCount, hexInsertMask, kernel::InsertPosition::After);
